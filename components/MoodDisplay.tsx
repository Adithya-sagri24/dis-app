...
export const MoodDisplay: React.FC = () => {
  ...
  const containerClasses = "p-6 bg-black/20 backdrop-blur-xl border border-white/10 rounded-2xl shadow-lg";

  if (!currentMood) {
    return (
      <div className={`${containerClasses} text-center`}>
        <p className="text-gray-400">Awaiting mood data...</p>
        <p className="text-xs text-gray-500 mt-2">Make sure your face is visible to the camera.</p>
      </div>
    );
  }
  ...
  return (
    <div className={`${containerClasses} animate-fade-in`}>
        <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-semibold text-gray-200">Current Mood</h3>
            <span className="text-4xl" role="img" aria-label={emotion}>{emoji}</span>
        </div>
        <div className="space-y-4">
            <ProgressBar label="Valence (Pleasantness)" value={valence} color="#818cf8" />
            <ProgressBar label="Energy (Arousal)" value={energy} color="#f472b6" />
        </div>
    </div>
  );
};